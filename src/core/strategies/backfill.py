...
class BackfillScheduler(BaseScheduler):
        ...
        while True:
            ...
            for job in self.jobs:
                if self._can_backfill(job, earliest_start):
                    ...
                    env.process(self._run_job(env, job))
                    break
                else:
                    ...
                    if self._can_schedule(primary_job):
                        ...
                        env.process(self._run_job(env, primary_job))
                    else:
                        yield env.timeout(1)
    
    def _run_job(self, env, job: Job):
        node = self._find_available_node(job)
        if not node:
            logger.warning(f"[{env.now}] No available node for job {job.name}")
            return

        # Set waiting time when job starts
    ...
    def _find_earliest_start(self, env, job: Job) -> int:
        # Find the earliest time a job can start based on current jobs in the queue
